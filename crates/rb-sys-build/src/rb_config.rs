...
///
/// This matches the format produced by RbConfigParser in rb-sys-cli phase_0.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SerializedRbConfig {
    /// The prefix path (relative to the JSON file location)
    pub prefix: String,
...
impl RbConfig {
    ...
    /// Load RbConfig from a JSON file (typically rbconfig.json generated by rb-sys-cli).
    ///
    /// This is used for pre-generating bindings where we can't execute the Ruby binary
    /// (e.g., cross-compilation targets). The JSON format matches what RbConfigParser
    /// produces in phase_0.
    ///
    /// # Arguments
    ///
    /// * `json_path` - Path to the rbconfig.json file
    ///
    /// # Returns
    ///
    /// An RbConfig populated from the JSON file's config values.
    pub fn from_json(json_path: &Path) -> Result<RbConfig, Box<dyn Error>> {
        let content = std::fs::read_to_string(json_path)?;
        let serialized: SerializedRbConfig = serde_json::from_str(&content)?;

        let mut rbconfig = RbConfig::new();

        // Populate the value map from the JSON config
        rbconfig.value_map = serialized.config;

        // Push cflags and dldflags if present
        if let Some(cflags) = rbconfig.value_map.get("cflags").cloned() {
            rbconfig.push_cflags(&cflags);
        }
        if let Some(dldflags) = rbconfig.value_map.get("DLDFLAGS").cloned() {
            rbconfig.push_dldflags(&dldflags);
        }
    ...
    fn test_from_json() {
        ...
        let json_content = r#"{
            ...
            "config": {
                ...
                "RUBY_SO_NAME": "ruby",
                "rubyhdrdir": "/usr/local/ruby/include/ruby-3.3.0",
                "rubyarchhdrdir": "/usr/local/ruby/include/ruby-3.3.0/aarch64-linux-gnu",
                "cflags": "-O2 -fPIC",
                "DLDFLAGS": "-L/usr/local/lib -shared"
            }
        }"#;

        let temp_dir = std::env::temp_dir();
        let json_path = temp_dir.join("test_rbconfig.json");
        let mut file = std::fs::File::create(&json_path).unwrap();
        file.write_all(json_content.as_bytes()).unwrap();

        let rbconfig = RbConfig::from_json(&json_path).unwrap();

        assert_eq!(rbconfig.get("MAJOR"), Some("3".to_string()));
        assert_eq!(rbconfig.get("MINOR"), Some("3".to_string()));
        assert_eq!(
            rbconfig.get("platform"),
            Some("aarch64-linux-gnu".to_string())
        );
        assert_eq!(rbconfig.platform(), "aarch64-linux-gnu");
        assert_eq!(rbconfig.major_minor(), Some((3, 3)));
        assert_eq!(rbconfig.cflags, vec!["-O2", "-fPIC"]);
        assert_eq!(
            rbconfig.search_paths,
            vec![SearchPath {
                kind: SearchPathKind::Native,
                name: "/usr/local/lib".to_string()
            }]
        );
