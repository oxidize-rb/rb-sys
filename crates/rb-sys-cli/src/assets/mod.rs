...
impl AssetManager {
    ...
    pub fn extract_sysroot(&self, ruby_platform: &str, dest_dir: &Path) -> Result<()> {
        ...
        let sysroot_prefix = format!("assets/{ruby_platform}/sysroot/");
        debug!(
            platform = %ruby_platform,
        ...
        // Don't fail if no sysroot files were found - this is expected for some platforms
        if extracted_count == 0 {
            debug!(
                platform = %ruby_platform,
                "No sysroot files found in tarball (this may be expected)"
            );
        }
        ...
        info!(
            ...
            "Extracted sysroot files"
        );

        if extracted_count == 0 {
            debug!(
                platform = %ruby_platform,
                "No sysroot files found for platform"
            );
        }
    ...
    pub fn extract_rubies(&self, ruby_platform: &str, dest_dir: &Path) -> Result<()> {
        // Check if already extracted (marker file)
        let marker = dest_dir.join(".rubies-extracted");
        if marker.exists() {
            debug!(
                platform = %ruby_platform,
                dest = %dest_dir.display(),
                "Rubies already extracted"
            );
            return Ok(());
        }
        ...
        let rubies_prefix = format!("assets/{ruby_platform}/rubies/");
        debug!(
            platform = %ruby_platform,
            prefix = %rubies_prefix,
            dest = %dest_dir.display(),
            "Extracting rubies"
        );
        ...
        for entry in archive.entries().context("Failed to read tar entries")? {
            ...
            // Only extract rubies files for this platform
            if let Some(relative) = path_str.strip_prefix(&rubies_prefix) {
                let dest_path = dest_dir.join(relative);
                extracted_count += 1;
        ...
        if extracted_count == 0 {
            debug!(
                platform = %ruby_platform,
                "No rubies found for platform (this may be expected if ruby_versions is empty)"
            );
        }
    ...
    pub fn extract_ruby_sysroot(
        &self,
        ruby_platform: &str,
        ruby_version: &str,
        dest_dir: &Path,
    ) -> Result<()> {
        let platform = self.manifest().platform_for_rust_target(ruby_platform)?;

        let sysroot_archive = platform.ruby_sysroot_archive.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "No Ruby sysroot archive available for platform: {}",
                ruby_platform
            )
        })?;

        // Check if already extracted (marker file)
        let marker = dest_dir.join(format!(".ruby-{}-extracted", ruby_version));
        if marker.exists() {
            debug!(
                platform = %ruby_platform,
                version = %ruby_version,
                dest = %dest_dir.display(),
                "Ruby sysroot already extracted"
            );
            return Ok(());
        }
        ...
        info!(
            platform = %ruby_platform,
            version = %ruby_version,
            "Extracting Ruby sysroot from embedded assets"
        );

        // Create temp directory for extraction
        let temp_dir = tempfile::tempdir_in(dest_dir)
            .context("Failed to create temp directory for Ruby sysroot extraction")?;

        // Extract Ruby sysroot directly from embedded rubies archive
        let temp_ruby_dir =
            self.extract_ruby_sysroot_archive(ruby_platform, ruby_version, temp_dir.path())?;

        // Copy the extracted Ruby sysroot to final destination
        self.copy_dir_recursively(&temp_ruby_dir, dest_dir)?;

        // Write marker file
        fs::write(
            &marker,
            format!(
                "Extracted Ruby {} for {} at {}",
                ruby_version,
                ruby_platform,
                chrono::Utc::now()
            ),
        ...
        info!(
            ...
            version = %ruby_version,
            dest = %dest_dir.display(),
            "Ruby sysroot extracted successfully"
        );
    ...
    fn extract_ruby_sysroot_archive(
        &self,
        ruby_platform: &str,
        ruby_version: &str,
        temp_dir: &Path,
    ) -> Result<PathBuf> {
        let rubies_data = get_embedded_rubies()?;
        let decoder = zstd::Decoder::new(rubies_data)
            .context("Failed to create zstd decoder for rubies archive")?;
        let mut archive = tar::Archive::new(decoder);

        let ruby_path = format!("{}/ruby-{}/", ruby_platform, ruby_version);
        let temp_ruby_dir = temp_dir.join("ruby_sysroot");

        fs::create_dir_all(&temp_ruby_dir).with_context(|| {
            format!(
                "Failed to create Ruby sysroot directory: {}",
                temp_ruby_dir.display()
            )
        })?;
        ...
        for entry in archive
            .entries()
            .context("Failed to read rubies tar entries")?
        {
            let mut entry = entry.context("Failed to read rubies tar entry")?;
            let path = entry.path().context("Failed to get rubies entry path")?;
            let path_str = path.to_string_lossy();

            // Extract files that belong to this Ruby version/platform
            if let Some(relative) = path_str.strip_prefix(&ruby_path) {
                if relative.is_empty() {
                    continue; // Skip the directory itself
                }

                let dest_path = temp_ruby_dir.join(relative);
