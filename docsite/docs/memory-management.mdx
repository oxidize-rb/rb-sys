...
This guide covers how Ruby's garbage collector interacts with Rust objects and how to prevent memory leaks and
segmentation faults in your extensions.
...
```rust
use magnus::{prelude::*, Error, Ruby, TypedData, DataTypeFunctions, function, method};
...
impl ErrorInfo {
    ...
    pub fn code(rb_self: Obj<Self>) -> Result<Option<Symbol>, Error> {
        let ruby = Ruby::get_with(rb_self);
        match &rb_self.kind {
            ErrorKind::StackOverflow => Ok(Some(ruby.to_symbol("STACK_OVERFLOW"))),
            ErrorKind::MemoryOutOfBounds => Ok(Some(ruby.to_symbol("MEMORY_OUT_OF_BOUNDS"))),
            ErrorKind::InvalidOperation(_) => Ok(Some(ruby.to_symbol("INVALID_OPERATION"))),
            ErrorKind::Unknown => Ok(Some(ruby.to_symbol("UNKNOWN"))),
        }
    }
    ...
    pub fn inspect(rb_self: Obj<Self>) -> Result<String, Error> {
        Ok(format!(
            ...
            ErrorInfo::code(rb_self)?.map_or("nil".to_string(), |s| s.to_string())
        ))
    }
}
...
impl MemorySlice {
    fn new(ruby: &Ruby, memory: Obj<Memory>, offset: usize, size: usize) -> Result<Self, Error> {
    ...
    fn to_str(&self) -> Result<RString, Error> {
        ...
        let slice = data.get(self.offset..self.offset + self.size)
            .ok_or_else(|| Error::new(ruby.exception_range_error(), "slice out of bounds"))?;
...
fn inefficient_string(ruby: &Ruby, data: &[u8]) -> Result<RString, Error> {
    let temp_string = String::from_utf8(data.to_vec()).map_err(|e| Error::new(ruby.exception_type_error(), e.to_string()))?; // Unnecessary allocation
...
impl Counter {
    fn buggy_add(&self, ruby: &Ruby, val: i64) -> Result<i64, Error> {
        let ruby = unsafe { Ruby::get_unchecked() };
...
impl MutGame {
    ...
    fn buggy_next_player_scores(&self, ruby: &Ruby, points: i64) -> Result<String, Error> {
        ...
        let player_name = game_mut.players.get(player).ok_or_else(|| {
            Error::new(ruby.exception_index_error(), "player index out of bounds")
        })?;

        Ok(format!("{} scored {} points! New total: {}",
            player_name, points, game_mut.score))
    }

    // CORRECT: Copy all needed data before releasing the borrow
    fn safe_next_player_scores(&self, ruby: &Ruby, points: i64) -> Result<String, Error> {
...

        {
            ...
            if game.players.is_empty() {
                            return Err(Error::new(
                                ruby.exception_runtime_error(),                    "No players in game"
                ));
            }

            player_name = game.players.get(game.current_player).ok_or_else(|| {
                Error::new(ruby.exception_index_error(), "player index out of bounds")
            })?.clone();
