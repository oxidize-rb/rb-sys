...
:::warning Improper error handling can lead to crashes that take down the entire Ruby VM. Learn how to properly raise
and handle exceptions in your Rust extensions. :::
...
:::note In rb-sys extensions, Rust's `Result<T, magnus::Error>` type is typically used, where the `Error` type
represents a Ruby exception. :::

## The Result Type and Magnus::Error

Magnus uses `Result&lt;T, Error&gt;` as the standard way to handle errors. The `Error` type represents a Ruby exception
that can be raised:
...
```rust
use magnus::{Error, Ruby, RArray, Value};
...
```rust
use magnus::{Error, Ruby, exception};
...
```rust
use magnus::{Error, Ruby, Value, RArray, value::ReprValue};
use std::fs::File;
...
:::note Ruby has a rich hierarchy of exception types. Using the specific exception type helps users handle errors
properly in their Ruby code. :::
...
:::warning Never use `unwrap()` or `expect()` in production code for your Ruby extensions. These can cause panics that
will crash the Ruby VM. Always use proper error handling with `Result` and `Error` types. :::
...
:::tip Test your extension with invalid inputs to ensure it throws appropriate Ruby exceptions instead of crashing. :::
