...
fn init() -> Result<(), Error> {
    let module = define_module("StringUtils")?;
...
```rust
use magnus::{RHash, RArray, Value, Error};
use serde_json;
...
```rust
use magnus::{class, define_class, method, Error, Ruby};
...
```rust
use magnus::{define_class, method, Error};
use std::sync::{Arc, Mutex};
