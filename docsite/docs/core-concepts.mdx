...
```rust
use magnus::{RHash, RArray, Value, Symbol};
...
fn init() -> Result<(), Error> {
    let module = define_module("MyMath")?;
...
fn safe_operation(input: String) -> Result<String, Error> {
    ...
    let doubled = count.checked_mul(2).ok_or_else(|| {
        Error::new(exception::range_error(), "overflow")
    })?;
